...
public class EntityMap implements Listener {
   ...
   public static EntityData get(Entity entity) {
      if (ModelEngineAPI.isModeledEntity(entity.getUniqueId())) {
         return (EntityData)ENTITIES.get(Bukkit.getEntity(ModelEngineAPI.getModeledEntity(entity.getUniqueId()).getBase().getUniqueId()));
      } else {
         return ModelEngineAPI.getModelTicker().getSubHitboxBone(entity.getUniqueId()) != null ? (EntityData)ENTITIES.get(Bukkit.getEntity(ModelEngineAPI.getModelTicker().getSubHitboxBone(entity.getUniqueId()).getActiveModel().getModeledEntity().getBase().getUniqueId())) : (EntityData)ENTITIES.get(entity);
      }
   }
